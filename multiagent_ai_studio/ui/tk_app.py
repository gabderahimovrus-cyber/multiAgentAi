...
from tkinter import BOTH, END, HORIZONTAL, LEFT, RIGHT, VERTICAL, WORD, BooleanVar, StringVar, Text, Tk, filedialog, messagebox, simpledialog
...
from ..core.models import Agent, AgentRole, DEFAULT_PERMISSIONS
...
class StudioWindow(Tk):
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(self.topbar, textvariable=self.status_text, style="Panel.TLabel").pack(side=LEFT, padx=20)
    ...
    def _build_left_panel(self) -> None:
        ttk.Label(self.left, text=self.t("left.chats"), style="Panel.TLabel").pack(anchor="w", padx=10, pady=(10, 3))
        self.chat_list = ttk.Treeview(self.left, show="tree", height=3)
        self.chat_list.pack(fill="x", padx=10)
        self.chat_list.insert("", END, iid="default", text="Основной чат")
        self.chat_list.selection_set("default")

        ttk.Label(self.left, text=self.t("left.projects"), style="Panel.TLabel").pack(anchor="w", padx=10, pady=(14, 3))
        ttk.Button(self.left, text=self.t("projects.new"), command=self.create_project).pack(fill="x", padx=10, pady=(0, 4))
        self.project_list = ttk.Treeview(self.left, show="tree", height=6)
        self.project_list.pack(fill="both", expand=False, padx=10)

        ttk.Label(self.left, text=self.t("left.agents"), style="Panel.TLabel").pack(anchor="w", padx=10, pady=(14, 3))
        btns = ttk.Frame(self.left, style="Panel.TFrame")
        btns.pack(fill="x", padx=10)
        ttk.Button(btns, text="+", width=3, command=self.create_agent).pack(side=LEFT, padx=(0, 3))
        ttk.Button(btns, text="⧉", width=3, command=self.clone_agent).pack(side=LEFT, padx=3)
        ttk.Button(btns, text="−", width=3, command=self.delete_agent).pack(side=LEFT, padx=3)
        self.agent_list = ttk.Treeview(self.left, show="tree", height=14)
        self.agent_list.pack(fill="both", expand=True, padx=10, pady=(4, 10))
    ...
    def _build_chat_panel(self) -> None:
        ...
        self.chat_title = ttk.Label(toolbar, text="Чат", style="Panel.TLabel", font=("Segoe UI", 12, "bold"))
        ...
        ttk.Button(entry_frame, text=self.t("chat.send"), style="Accent.TButton", command=self.send_message).pack(side=RIGHT, padx=(8, 0), fill="y")

    def _build_right_panel(self) -> None:
        ttk.Label(self.right, text=self.t("agent.settings"), style="Panel.TLabel", font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=10, pady=(10, 6))
        ...
        self._labeled_entry(form, self.t("agent.name"), self.agent_name)
        self._labeled_combo(form, self.t("agent.role"), self.agent_role, [role.value for role in AgentRole])
        self._labeled_combo(form, self.t("agent.model"), self.agent_model, [])
        self._labeled_entry(form, self.t("agent.temperature"), self.agent_temp)
        ttk.Checkbutton(form, text=self.t("agent.enabled"), variable=self.agent_enabled).pack(anchor="w", pady=4)
        ttk.Label(form, text=self.t("agent.system_prompt"), style="Panel.TLabel").pack(anchor="w", pady=(8, 2))
        self.system_prompt = Text(form, height=7, wrap=WORD)
        self.system_prompt.pack(fill="x")
        ttk.Label(form, text=self.t("agent.permissions"), style="Panel.TLabel").pack(anchor="w", pady=(8, 2))
        self.permission_vars: dict[str, BooleanVar] = {}
        perms = ttk.Frame(form, style="Panel.TFrame")
        perms.pack(fill="x")
        for i, perm in enumerate(DEFAULT_PERMISSIONS):
            var = BooleanVar(value=DEFAULT_PERMISSIONS[perm])
            self.permission_vars[perm] = var
            ttk.Checkbutton(perms, text=perm, variable=var).grid(row=i // 2, column=i % 2, sticky="w", padx=2, pady=1)
        ttk.Button(form, text=self.t("agent.refresh_models"), command=self.refresh_models).pack(fill="x", pady=(10, 3))
        ttk.Button(form, text=self.t("agent.save"), command=self.save_agent).pack(fill="x", pady=3)
        ttk.Label(form, text="Рабочая папка агента, память и параметры сохраняются в SQLite и на диск.", style="Muted.TLabel", wraplength=300).pack(anchor="w", pady=10)

    def _build_log_panel(self) -> None:
        toolbar = ttk.Frame(self.log_frame, style="Panel.TFrame")
        toolbar.pack(fill="x", padx=10, pady=(8, 4))
        ttk.Label(toolbar, text=self.t("logs.title"), style="Panel.TLabel", font=("Segoe UI", 11, "bold")).pack(side=LEFT)
        self.log_filter = StringVar()
        ttk.Entry(toolbar, textvariable=self.log_filter, width=24).pack(side=LEFT, padx=8)
        ...
        self.log_text = Text(self.log_frame, height=8, wrap=WORD, borderwidth=0, padx=8, pady=8)
        self.log_text.pack(fill=BOTH, expand=True, padx=10, pady=(0, 10))
        self.log_text.configure(state="disabled", bg=self.colors["entry"], fg=self.colors["text"], font=("Consolas", 9))

    def _labeled_entry(self, parent, label: str, var: StringVar) -> None:
        ttk.Label(parent, text=label, style="Panel.TLabel").pack(anchor="w", pady=(6, 2))
    ...
    def _bootstrap_data(self) -> None:
        ...
        default_model = self.ollama_models[0] if self.ollama_models else ""
    ...
    def refresh_agents(self) -> None:
        ...
        for agent in agents:
            ...
            parent = str(agent.parent_id) if agent.parent_id else ""
            iid = str(agent.id)
            self.agent_list.insert(parent if parent in self.agent_list.get_children("") else "", END, iid=iid, text=f"{marker} {agent.name} — {agent.role}")
    ...
    def refresh_logs(self) -> None:
        ...
        for event in self.db.list_logs(text_filter=self.log_filter.get()):
            self.log_text.insert(END, f"[{event.created_at}] {event.level} {event.event_type}: {event.message}\n")
    ...
    def on_agent_selected(self) -> None:
        ...
        if agent:
            ...
            self.chat_title.configure(text=f"Чат с агентом: {agent.name}")
    ...
    def create_agent(self) -> None:
        ...
        agent = self.agent_service.create_agent(name, role, self.ollama_models[0] if self.ollama_models else "")
        self.current_agent = agent
        self.refresh_agents()
    ...
    def send_message(self) -> None:
        ...
        threading.Thread(target=self._generate_response, args=(agent, text), daemon=True).start()
    ...
    def _generate_response(self, agent: Agent, text: str) -> None:
        try:
            answer = self.agent_service.send_user_message(agent, self.chat_id, text)
            self.after(0, self.refresh_chat)
        except Exception as exc:  # noqa: BLE001
            self.logs.log("model.error", str(exc), agent_id=agent.id, level="ERROR")
    ...
    def update_activity(self) -> None:
        active = sum(1 for agent in self.db.list_agents() if agent.enabled)
        self.activity_text.set(f"Агенты: {active} активных")

    def change_theme(self) -> None:
        self.db.set_setting("theme", self.theme.get())
        self.apply_theme()
        # Reconfigure text widgets that ttk styles do not control.
        for widget in (getattr(self, "chat_text", None), getattr(self, "message_entry", None), getattr(self, "log_text", None), getattr(self, "system_prompt", None)):
            if widget:
